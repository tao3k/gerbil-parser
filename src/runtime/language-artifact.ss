...
;; : (forall (a) (-> String List U8Vector String [(Pair Symbol a)]))
;; : (-> String List U8Vector String Alist)
...
(def (load-compiled-language-artifact/roots expected-schema locator roots)
  ...
  (unless (and (list? roots)
    ...
    (error "invalid compiled language artifact roots" roots))
  (let* ((relative-path (car locator))
         (path
          (or (find file-exists?
                    (map (cut path-expand relative-path <>) roots))
              (error "compiled language artifact sidecar is unavailable"
                     expected-schema relative-path))))
    (decode-compiled-language-artifact
     expected-schema locator
     (call-with-input-file path read-all-as-u8vector) path)))
