((max_total . 100ms)
 (maxCollectMs . 25)
 (maxParseMs . 15)
 (maxFileMs . 5)
 (maxPhaseMs . 6)
 (observedCollectMs . 0)
 (observedParseMs . 10)
 (observedFileMs . 0)
 (observedPhaseMs . 0)
 (observed_total . 10ms)
 (target_total . 25ms)
 (regression_budget . 15ms)
 (expected_over_input_budget . 15ms)
 (observedTimings
  ((name . collect-before) (durationMs . 0))
  ((name . collect-after) (durationMs . 0))
  ((name . policy-before) (durationMs . 10))
  ((name . policy-after) (durationMs . 0)))
 (targetRationale . "all 12 accepted HCL specsuite sources, all 14 openGQL examples, and three admitted TLA+ modules must publish lossless ParseArtifacts within the cross-language budget")
 (maxRssMb . 512)
 (memoryMetric . resident-set-size)
 (memoryUnit . "MB")
 (iterations . 5)
 (unit . "ms")
 (rule . GERBIL-PARSER-VERSIONED-LANGUAGES)
 (feature . versioned-language-contracts)
 (optimizationFocus . "one generic generated machine across configuration and graph-query syntax families")
 (inputShape . "the admitted corpus: 12 HCL v2.24.0 sources, 14 openGQL 1.9.0 examples, and three TLA+ 1.5 modules per sample")
 (expectedOutcome . "all pinned language contracts roundtrip with no runtime language dispatch")
 (measurementPhases collect-before collect-after policy-before policy-after
                    assert-time-gate assert-memory-gate)
 (tags performance hot-path hcl gql tla-plus generic-parser))
