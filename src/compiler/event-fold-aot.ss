...
(def (fold-predicate expression line start end states indices)
         ...
         (if (eq? (car expression) 'line-bytes-all-in?)
           ...
           (and (< cursor until)
                (or (memv (u8vector-ref bytes (- cursor start))
                          (cadddr expression))
                    (loop (+ cursor 1))))))))
    ((not) (not (fold-predicate (cadr expression) line start end states indices)))
    ((and) (and (fold-predicate (cadr expression) line start end states indices)
                (fold-predicate (caddr expression) line start end states indices)))
    ((or) (or (fold-predicate (cadr expression) line start end states indices)
              (fold-predicate (caddr expression) line start end states indices)))
...
(def (fold-statements statements line start end states indices)
    ...
    (if (null? rest) (cons state events)
        (let* ((form (car rest))
               (step
                (case (car form)
                  ((set-bool)
                   (let (value (fold-predicate (caddr form) line start end state indices))
                  ...
                  ((if)
                   (fold-statements
                    (if (fold-predicate (cadr form) line start end state indices)
                      (caddr form) (cadddr form))
                    line start end state indices))
                  ((for-line-bytes)
                   (let ((from (fold-offset (caddr form) line start end state indices))
                     ...
                     (let iterate ((cursor from) (current state) (reversed []))
                       (if (= cursor until) (cons current (reverse reversed))
                         (let (step (fold-statements (list-ref form 4) line start end
                                                      current
                   ...
                   (let (closes (fold-frame-finishes (list-ref form 3)))
                     (let close ((current state) (reversed []))
                       (let (stack (fold-state-of-type current (cadr form) list?))
                         (if (and (pair? stack)
                                  (fold-predicate (caddr form) line start end current indices))
  ...
  (case (car expression)
    ...
    ((and or)
     (hash ("kind" (symbol->string (car expression)))
           ("left" (fold-predicate-ir (cadr expression) states indices allow-line?))
           ("right" (fold-predicate-ir (caddr expression) states indices allow-line?))))
    (else (error "unsupported event fold predicate" expression))))
