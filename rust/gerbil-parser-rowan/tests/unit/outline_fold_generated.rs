pub const PARSER_DIGEST: &str =
    "sha256:2f4a0b470cdfd6086248d45baf7725d0b1b71de98878f868c32eb0ae0b58b729";
    ...
    while start < bytes.len() {
        ...
        let __event_marker_42_32 = {
            let run = line
                .as_bytes()
                .iter()
                .take_while(|byte| **byte == 42u8)
                .count();
            if run > 0 && line.as_bytes().get(run) == Some(&32u8) {
                run
            } else {
                0
            }
        ...
        if (__event_marker_42_32) > 0 {
            while open_levels
                .last()
                .is_some_and(|&open| open >= __event_marker_42_32)
            {
